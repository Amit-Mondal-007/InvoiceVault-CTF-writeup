...
InvoiceVault is a six-person SaaS platform serving around four thousand freelancer accounts. A new account-export endpoint was scaffolded from an internal admin tool during a rushed week before a customer data-portability deadline. The engineer filed a follow-up security ticket the same afternoon — it has been sitting at the bottom of the backlog ever since.
...
The server generates and returns a ZIP archive for the account matching that ID — without verifying that the requesting user owns that account. Any authenticated user can export any other user's data simply by changing the `user_id` value.

The root cause: the endpoint was originally built as an internal admin tool where any user ID was valid. When it was repurposed for customer-facing data portability, the ownership check was never added.
